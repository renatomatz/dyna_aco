...
from env import McCallModel
from agent import Agent, EGreedyDeterministicDecision, MonteCarloUpdate, \
...
logger.add_log(Agent,
               AggLog(LogPrint(),
                      lambda utilities: " | Average Utility: "
                                        f" {str(np.mean(utilities))}\n"),
               lambda it_agent: it_ant.to_date_utility())
logger.add_log(TabularQOptimizer,
...
opt.agent_gen = Agent.static_generator(
    T, alpha=1.0, beta=0.5, rho=0.1,
    decision=decision,
    update=update
)

constagent_beliefs = True
if constagent_beliefs:
